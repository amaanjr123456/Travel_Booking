...
document.getElementById('bookingform').addEventListener('submit', async (e) => {
    ...
    const data = {
        ...
        date: formData.get('date'), // Ensure your HTML input 'name' is 'date'
        destination: formData.get('destination'),
        notes: formData.get('additional notes') // Matches your HTML 'name' attribute
    };
    ...
    try {
        ...
        if (response.ok) {
            // SUCCESS: Display the unique Booking ID (TBK-XXXXX)
        ...
        alert("Server error. Check Vercel Logs for database connection issues.");
    }
});
